...
if __name__ == '__main__':
    ...
    if os.path.exists('transformer_' + str(version) + '.pt'):
        model1 = torch.load('transformer_' + str(version) + '.pt')
    else:
        model1 = Transformer(dm=dm, vocab_size=vocab_size,block_size=block_size, h=h, N=N, block_type=0)
    print(sum(p.numel() for p in model1.parameters())/1e6, 'M parameters')
    m1=model.to(device)
    optimizer1 = torch.optim.AdamW(model1.parameters(), lr=lr)
    
    if os.path.exists('transformer_' + str(version) + '.pt'):
        model2 = torch.load('transformer_' + str(version) + '.pt')
    else:
        model2 = Transformer(dm=dm, vocab_size=vocab_size,block_size=block_size, h=h, N=N, block_type=2)
    print(sum(p.numel() for p in model2.parameters())/1e6, 'M parameters')
    m2=model.to(device)
    optimizer1 = torch.optim.AdamW(model2.parameters(), lr=lr)
